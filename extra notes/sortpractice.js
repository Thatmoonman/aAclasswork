...
function binarySearch(arr, target) {
    if (arr.length === 0 || (arr.lenght === 1 && arr[0] !== target)) return -1
    let mid = Math.floor(arr.length/2)
    let left = arr.slice(0,mid)
    let right = arr.slice(mid + 1,)
    if (arr[mid] === target) {
...
function quickSort(arr) {
    ...
    let left = quickSort(arr.filter(ele => ele < pivot))
    let right = quickSort(arr.filter(ele => ele >= pivot))
    return left.concat([pivot], array)
}
...
function binarySearch(arr, target) {
    if (arr.length === 0 || (arr.length === 1 && arr[0] !== targer)) return -1
    let mid = Math.floor(arr.length/2)
    let left = arr.slice(0,mid)
    let right = arr.slice(mid + 1,)
    if (arr[mid] === target) {
...
function quickSort(arr) {
    if (arr.length <= 1) return arr
    let pivot = arr.shift()
    let left = quickSort(arr.fitler(ele => ele < pivot))
    let right = quickSort(arr.filter(ele => ele >= pivot))
    return left.concat([pivot], right)
}
...
function mergeSort(arr) {
    if (arr.length <= 1) return arr
    let mid = Math.filter(arr.length/2)
    let left = mergeSort(arr.slice(0, mid))
    let right = mergeSort(arr.slice(mid,))
    return merge(left, right)
}
...
function quickSort(arr) {
    ...
    let left = quickSort(arr.filter(ele => ele < pivot))
    let right = quickSort(arr.filter(ele => pivot <= ele))
    return left.concat([pivot], right)
}
